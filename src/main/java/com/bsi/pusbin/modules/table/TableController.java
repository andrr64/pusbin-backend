...
public class TableController {
    ...
    public ResponseEntity<APIResponse<List<List<Object>>>> getWilayahKerjaTable(FilterRequest request) {
        List<List<Object>> data = tableService.getWilayahKerjaTable(request);
        return ResponseEntity.ok(APIResponse.ok(data));
    }

    @GetMapping({"/jabatan"})
    public ResponseEntity<APIResponse<List<List<Object>>>> getJabatanTable(FilterRequest request) {
        List<List<Object>> data = tableService.getJabatanTable(request);
        return ResponseEntity.ok(APIResponse.ok(data));
    }

    @GetMapping({"/pendidikan"})
    public ResponseEntity<APIResponse<List<List<Object>>>> getPendidikanTable(FilterRequest request) {
        List<List<Object>> data = tableService.getPendidikanTable(request);
        return ResponseEntity.ok(APIResponse.ok(data));
    }

    @GetMapping({"/instansi"})
    public ResponseEntity<APIResponse<List<List<Object>>>> getInstansiTable(FilterRequest request) {
